  ...
  <aside class="w-64 bg-gray-800 text-gray-100 shadow-lg flex flex-col">
    ...
    <div class="flex-1 flex flex-col">
      ...
      <main class="p-6">
        ...
        <h2 class="text-2xl font-semibold text-gray-700">👩‍🎓Data Ketua Jurusan</h2>
        </div>
          <!-- Tombol Tambah Data di kiri atas -->
          <div class="flex justify-between items-center mb-4">
          <a href="{{ route('kajur.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2 rounded-md text-sm transition">+ Tambah Data</a>
            <!-- Search Bar, supaya bisa langsung aktif ditambahin id=serchinput --> 
            <div class="relative w-64">
            <input id="searchInput" type="text" placeholder="Cari Kajur..." class="pl-10 pr-4 py-2 w-full border rounded-md focus:outline-none focus:ring focus:ring-indigo-300 text-sm">
          ...
          <table class="min-w-full border border-gray-300 text-sm rounded-md overflow-hidden">
            <thead>
              <tr class="bg-gray-100 text-center text-gray-600">
                ...
                <th class="px-5 py-3 border-b border-gray-300">Nama Jurusan</th>
            ...
            <tbody id="kajurTableBody"> 
              @foreach ($kajur as $kjr)
              <tr class="hover:bg-gray-50 text-center">
                <td class="px-5 py-3 border-b border-gray-200">{{ $kjr['id_kajur']}}</td>
                <td class="px-5 py-3 border-b border-gray-200">{{ $kjr['nama_kajur']}}</td>
                <td class="px-5 py-3 border-b border-gray-200">{{ $kjr['nidn']}}</td>
                <td class="px-5 py-3 border-b border-gray-200">{{ $kjr['nama_jurusan']}}</td>
                <td class="px-5 py-3 border-b border-gray-200">{{ $kjr['id_user']}}</td>
                <td class="px-5 py-3 border-b border-gray-200 flex justify-center space-x-3">
                  <!-- Tombol Edit -->
                  <a href="{{ route('kajur.edit', $kjr['id_kajur']) }}" class="bg-yellow-500 text-white px-2 py-1 rounded">✏️Edit</a>
                  <!-- Tombol Hapus -->
                  <form action="{{ route('kajur.destroy', $kjr['id_kajur']) }}" method="POST" style="display:inline;">
                  @csrf
                  @method('DELETE')
...
<script> //script js supaya bisa langsung serch
  document.addEventListener("DOMContentLoaded", function () {
    ...
    const tableBody = document.getElementById("kajurTableBody");
